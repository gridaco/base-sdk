...
const axios = Axios.create({
    baseURL: __HOSTS.ASSET_SERVICE_HOST,
});

export async function registerVariantAsset(
    projectId: string,
    req: VariantAssetRegisterRequest
): Promise<VariantAssetRegisterResponse> {
...
export async function putVariant(
    projectId: string,
    request: VariantPutRequest
): Promise<VariantAsset> {
    try {
        const resp = await axios.put(
            `${VARIANT_ASSETS_ROUTE}/${request.variantAssetId}/variants/${request.variant}`,
            request.asset
        );
        return resp.data as VariantAsset;
    } catch (_) {
