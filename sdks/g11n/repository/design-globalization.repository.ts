...
export class DesignGlobalizationRepository {
    ...
    constructor(readonly projectId: string, readonly sceneId: string) {
        this.textTranslationRepository = new TextTranslationRepository(
            projectId
        );
        this.layerKeyMapRepository = new LayerKeyMapRepository(
            projectId,
            sceneId
        );
    }

    async registerTextKey(
        layerId: string,
        request: GlobalizedTextKeyRegisterRequest
    ) {
    ...
    async putTextTranslation(
        layerId: string,
        request: TextTranslationPutRequest
    ) {
        const key = await this.textTranslationRepository.putTranslation(
            request
        );
        return key;
    }
    ...
    async fetchTranslation(
        ...
        const lt = await this.layerKeyMapRepository.fetchLayerTranslation(
            layerId
        );
        if (lt) {
            const translation = lt.translation;
            console.log(
                `fetched translation for layer ${layerId}`,
                translation
            );
            return translation;
        }
    ...
    async fetchKeys(): Promise<ReadonlyArray<string>> {
        return (await this.layerKeyMapRepository.fetchTranslations()).map(
            (e) => e.keyId
        );
    }
